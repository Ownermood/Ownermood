...
_DEFAULT_PLAN_CONTENT = (
    "🎉 <b>Welcome {mention} to Sugar Daddy DRM Bot!</b> 🎉\n\n"
    "🔐 You can have access to download all <b>NON-DRM + AES Encrypted URLs</b> including:\n\n"
    "🌟 <b>LOGIN REQUIRED</b> 🌟\n"
    "🎯 AppX Zip + Encrypted upto 4 UHS\n"
    "🎯 ClassPlus DRM + NDRM\n"
    "🎯 CareerWill + PDF\n"
    "🎯 Khan GS\n"
    "🎯 Allen\n"
    ...
    "🎯 MPD URLs if the key is known\n\n"
    "🌿 <b>WITHOUT LOGIN — ALL BATCHES</b> 🌿\n"
    "🎯 Study IQ\n"
    "🎯 PhysicsWallah\n"
    "🎯 Unacademy\n"
    "🎯 KGS (Khan Sir)\n"
    ...
    "🎯 Legal Adda\n"
    "🎯 Adda247\n\n"
    "💰 If you want to buy subscription of the bot, <b>₹2,500/Month</b> — feel free to contact "
    "🌷 <a href='{credit_link}'>{credit}</a> 🌷"
)
...
async def show_plan_for_user_msg(bot_client, m, bot_username: str):
    """/plan command — shows subscription status only."""
    user_id    = (m.from_user.id if m.from_user else None) or m.chat.id
    first_name = (m.from_user.first_name if m.from_user else None) or "User"
    import html as _h
    _mention   = f'<a href="tg://user?id={user_id}">{_h.escape(first_name)}</a>'
    _is_owner  = user_id in {OWNER, OWNER_ID, OWNER_ID2}
    is_premium = _is_owner or db.is_user_authorized(user_id, bot_username)
        ...
        if not _is_owner:
            try:
                ...
                if _info:
                    ...
                    _details = (
                        f"<blockquote>📋 Status  ›  <b>{_s}</b>\n"
                        f"📅 Expiry  ›  <b>{_expiry}</b>\n"
                        f"⏳ Remaining  ›  <b>{_days} days</b></blockquote>\n\n"
                    )
            except Exception:
                pass
        text = f"📋 <b>Subscription</b>\n\n👤 {_mention}\n✅ <b>Premium Member</b>\n\n{_details}👤 <a href='{CREDIT_LINK}'>{CREDIT}</a>"
    else:
        text = f"📋 <b>Subscription</b>\n\n👤 {_mention}\n❌ <b>Not subscribed</b>\n\n👤 <a href='{CREDIT_LINK}'>{CREDIT}</a>"
        keyboard = InlineKeyboardMarkup([
...
async def _startup():
    ...
    # Always sync default plan content from code on startup
    try:
        db.set_setting("default_plan_content", _DEFAULT_PLAN_CONTENT)
        logging.warning("[STARTUP] Default plan content synced to DB")
    except Exception as _pe:
        logging.warning(f"[STARTUP] Could not sync plan: {_pe}")
