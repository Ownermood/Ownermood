...
class Database:
    ...
    # ==================== PLAN MANAGEMENT ====================
    # Schema: plan_name, duration_days, price, description, content,
    #         status (active/disabled), created_at, updated_at

    def get_all_plans(self) -> list:
        """Return all plans sorted by creation date."""
        try:
            return list(self.db["plans"].find({}, {"_id": 0}).sort("created_at", 1))
        except Exception as e:
            print(f"{Fore.RED}get_all_plans error: {e}{Style.RESET_ALL}")
            return []

    def get_plan(self, plan_name: str) -> dict | None:
        """Get a single plan by name."""
        try:
            return self.db["plans"].find_one({"plan_name": plan_name}, {"_id": 0})
        except Exception as e:
            print(f"{Fore.RED}get_plan error: {e}{Style.RESET_ALL}")
            return None

    def add_plan(self, plan_name: str, duration_days: int, price: str,
                 description: str = "", content: str = "") -> bool:
        """Add or update a plan. Content stored as HTML (entity-converted)."""
        try:
            now = datetime.now()
            ...
            print(f"{Fore.RED}add_plan error: {e}{Style.RESET_ALL}")
            return False

    def update_plan_field(self, plan_name: str, field: str, value) -> bool:
        """Update a single field of an existing plan."""
        try:
            ...
            print(f"{Fore.RED}update_plan_field error: {e}{Style.RESET_ALL}")
            return False

    def rename_plan(self, old_name: str, new_name: str) -> bool:
        """Rename a plan."""
        try:
            result = self.db["plans"].update_one(
                {"plan_name": old_name},
                {"$set": {"plan_name": new_name, "updated_at": datetime.now()}},
            )
            return result.matched_count > 0
        except Exception as e:
            print(f"{Fore.RED}rename_plan error: {e}{Style.RESET_ALL}")
            return False

    def delete_plan(self, plan_name: str) -> bool:
        """Delete a plan by name."""
        try:
            result = self.db["plans"].delete_one({"plan_name": plan_name})
            return result.deleted_count > 0
        except Exception as e:
            print(f"{Fore.RED}delete_plan error: {e}{Style.RESET_ALL}")
            return False
    ...
    def get_setting(self, key: str) -> str | None:
        ...
        try:
            ...
            print(f"{Fore.RED}get_setting error: {e}{Style.RESET_ALL}")
            return None
    ...
    def set_setting(self, key: str, value: str) -> bool:
        ...
        try:
            ...
            print(f"{Fore.RED}set_setting error: {e}{Style.RESET_ALL}")
            return False

    # ==================== END OF PLAN MANAGEMENT ====================
